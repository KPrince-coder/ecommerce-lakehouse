...
from etl.common.schemas import RAW_ORDER_ITEMS_SCHEMA
...
def write_order_items_data(df: DataFrame, bucket_name: str) -> None:
    """
...
def register_order_items_table(spark: SparkSession, bucket_name: str) -> None:
    """
    ...
    try:
        ...
        if success:
            logger.info(
                "Successfully registered order items table in Glue Data Catalog"
            )
        else:
...
def main(date: str, bucket_name: str, region: str, days_back: int = 1) -> int:
    ...
    try:
        ...
        spark = create_spark_session(
            app_name=f"bronze_order_items_etl_{date}", enable_hive_support=True
        )
