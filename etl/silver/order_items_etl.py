...
from pyspark.sql.functions import (
    col, lit, current_timestamp, when, dayofweek, hour, 
    to_date, date_format, coalesce, expr
)
...
def transform_order_items_data(df: DataFrame) -> DataFrame:
    ...
    try:
        # Add derived columns
        df_with_derived = add_derived_columns(df)
...
def write_silver_order_items(
    ...
    try:
        ...
        logger.info(f"Successfully wrote order items data to silver layer: {table_path}")
    except Exception as e:
        logger.error(f"Error writing order items data to silver layer: {str(e)}")
        raise


def register_silver_order_items_table(
    spark: SparkSession,
    bucket_name: str
) -> None:
    """
    ...
    try:
        ...
        if success:
            logger.info("Successfully registered silver order items table in Glue Data Catalog")
        else:
            logger.error("Failed to register silver order items table in Glue Data Catalog")
    except Exception as e:
...
def main(
    ...
    try:
        ...
        spark = create_spark_session(
            app_name=f"silver_order_items_etl_{date}",
