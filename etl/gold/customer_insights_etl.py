...
from pyspark.sql.functions import (
    ...
    when, datediff, max, min, concat_ws, to_date, current_date,
    ntile, expr
)
...
def write_gold_customer_insights(
    df: DataFrame,
    bucket_name: str,
    date: str
) -> None:
    ...
    try:
        ...
        logger.info(f"Successfully wrote customer insights data to gold layer: {table_path}")
    except Exception as e:
...
def register_gold_customer_insights_table(
    ...
    try:
        ...
        if success:
            logger.info("Successfully registered gold customer insights table in Glue Data Catalog")
        else:
            logger.error("Failed to register gold customer insights table in Glue Data Catalog")
    except Exception as e:
...
def main(
    ...
    try:
        ...
        spark = create_spark_session(
            app_name=f"gold_customer_insights_etl_{date}",
        ...
        logger.info(f"Successfully completed Gold Customer Insights ETL for date: {date}")
        return 0
    except Exception as e:
