...
from pyspark.sql.functions import (
    col, lit, current_timestamp, count, sum, avg, round, 
    when, countDistinct, max, min, concat_ws
)
...
def read_silver_data(
    ...
    try:
        ...
        order_items_df = read_delta_table(
            spark=spark,
            table_path=order_items_path,
            bucket_name=bucket_name
        ).filter(
            (col("date") >= start_date_str) & (col("date") <= date)
        )
...
def calculate_department_analytics(
    ...
    try:
        ...
        result_df = result_df.withColumn(
            ...
            lit("products,orders,order_items")
        )
...
def write_gold_department_analytics(
    df: DataFrame,
    bucket_name: str,
    date: str
) -> None:
    ...
    try:
        ...
        logger.info(f"Successfully wrote department analytics data to gold layer: {table_path}")
    except Exception as e:
...
def register_gold_department_analytics_table(
    ...
    try:
        ...
        if success:
            logger.info("Successfully registered gold department analytics table in Glue Data Catalog")
        else:
            logger.error("Failed to register gold department analytics table in Glue Data Catalog")
    except Exception as e:
...
def main(
    ...
    try:
        ...
        spark = create_spark_session(
            app_name=f"gold_department_analytics_etl_{date}",
        ...
        logger.info(f"Successfully completed Gold Department Analytics ETL for date: {date}")
        return 0
    except Exception as e:
