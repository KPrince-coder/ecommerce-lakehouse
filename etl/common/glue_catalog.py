...
def register_delta_table(
    ...
    try:
        ...
        if description:
            spark.sql(f"COMMENT ON TABLE {database_name}.{table_name} IS '{description}'")
        ...
        logger.error(f"Failed to register Delta table {database_name}.{table_name}: {str(e)}")
        return False
...
def update_table_partitions(
    ...
    try:
        ...
        logger.info(f"Started partition update for {database_name}.{table_name} with job ID: {job_run_id}")
        
        # Wait for the job to complete
        status = 'STARTING'
        while status in ['STARTING', 'RUNNING']:
            time.sleep(5)
            response = glue_client.get_partition_detection_run(
                JobRunId=job_run_id
            )
            status = response['Status']
        
        if status == 'SUCCEEDED':
            logger.info(f"Successfully updated partitions for {database_name}.{table_name}")
            return True
        else:
            logger.error(f"Failed to update partitions for {database_name}.{table_name}: {status}")
            return False
    except ClientError as e:
        logger.error(f"Error updating partitions for {database_name}.{table_name}: {str(e)}")
        return False
...
def get_table_metadata(
    ...
    try:
        ...
        if error_code == 'EntityNotFoundException':
            ...
            logger.error(f"Error getting metadata for {database_name}.{table_name}: {str(e)}")
            return None
