...
class TestBronzeOrderItemsETL(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.spark = SparkSession.builder \
            .appName("test_bronze_order_items_etl") \
            .master("local[1]") \
            .getOrCreate()
        
        # Create a sample order items DataFrame
        data = [
            (1, "10001", 1001, 5, 101, 1, True, datetime.strptime("2023-05-15 10:30:00", "%Y-%m-%d %H:%M:%S"), datetime.strptime("2023-05-15", "%Y-%m-%d").date()),
            (2, "10001", 1001, 5, 102, 2, False, datetime.strptime("2023-05-15 10:30:00", "%Y-%m-%d %H:%M:%S"), datetime.strptime("2023-05-15", "%Y-%m-%d").date()),
            (3, "10002", 1002, 10, 103, 1, True, datetime.strptime("2023-05-15 11:45:00", "%Y-%m-%d %H:%M:%S"), datetime.strptime("2023-05-15", "%Y-%m-%d").date()),
            (4, "10003", 1003, 7, 104, 1, False, datetime.strptime("2023-05-16 09:15:00", "%Y-%m-%d %H:%M:%S"), datetime.strptime("2023-05-16", "%Y-%m-%d").date()),
        ]
    ...
    @patch("etl.bronze.order_items_etl.spark")
    def test_read_order_items_data(self, mock_spark):
        ...
        # Call the function
        result_df = read_order_items_data(mock_spark, "test-bucket", ["2023-05-15", "2023-05-16"])
        ...
        mock_spark.read.format.return_value.option.return_value.option.return_value.schema.assert_called_with(RAW_ORDER_ITEMS_SCHEMA)
    ...
    def test_transform_order_items_data(self, mock_add_metadata, mock_validate_schema):
        ...
        mock_validate_schema.assert_called_once_with(self.sample_df, RAW_ORDER_ITEMS_SCHEMA, strict=True)
