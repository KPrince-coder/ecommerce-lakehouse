...
class TestBronzeOrdersETL(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.spark = (
            SparkSession.builder.appName("test_bronze_orders_etl")
            .master("local[1]")
            .getOrCreate()
        )
    ...
    @patch("etl.bronze.orders_etl.logger")
    def test_read_orders_data(self, mock_logger):
    ...
    def test_transform_orders_data(self, mock_add_metadata, mock_validate_schema):
        ...
        mock_validate_schema.assert_called_once_with(
            self.sample_df, RAW_ORDERS_SCHEMA, strict=True
        )
