"""
Lambda function to trigger the Gold Product Performance ETL Glue job.

This function is triggered by the Step Functions state machine and starts
the Glue job that runs the Gold Product Performance ETL process.
"""
...
GLUE_JOB_NAME = os.environ.get("GLUE_JOB_NAME", "gold-product-performance-etl")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Started Glue job gold-product-performance-etl with run ID {job_run_id}")

        # Wait for the job to complete
        job_status = wait_for_job_completion(job_run_id)

        if job_status == "SUCCEEDED":
            logger.info(f"Glue job gold-product-performance-etl completed successfully")
            return {
                "statusCode": 200,
                "jobRunId": job_run_id,
                "status": "SUCCEEDED",
                "message": f"Glue job gold-product-performance-etl completed successfully",
            }
        else:
            logger.error(f"Glue job gold-product-performance-etl failed with status {job_status}")
            return {
                "statusCode": 500,
                "jobRunId": job_run_id,
                "status": "FAILED",
                "message": f"Glue job gold-product-performance-etl failed with status {job_status}",
            }
