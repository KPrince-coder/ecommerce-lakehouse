#!/usr/bin/env python
"""
Run All Silver Layer ETL Processes

This script runs all Silver layer ETL processes in sequence:
1. Products ETL
2. Orders ETL
3. Order Items ETL

Usage:
    python scripts/run_silver_etl.py [--date YYYY-MM-DD] [--bucket-name BUCKET_NAME] [--region REGION] [--days-back DAYS]
"""
...
from etl.silver.products_etl import main as products_main
from etl.silver.orders_etl import main as orders_main
from etl.silver.order_items_etl import main as order_items_main
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run All Silver Layer ETL Processes"
    )
    parser.add_argument(
        "--date",
        type=str,
...
def main(date: str, bucket_name: str, region: str, days_back: int) -> int:
    """
    Main function to run all Silver layer ETL processes.
    ...
    """
    logger.info(f"Starting all Silver layer ETL processes for date: {date}")
    ...
    logger.info("All Silver layer ETL processes completed successfully")
    return 0


if __name__ == "__main__":
    args = parse_arguments()
    logger.info(f"Running all Silver layer ETL processes for date: {args.date}, days back: {args.days_back}")
    exit_code = main(args.date, args.bucket_name, args.region, args.days_back)
    logger.info(f"All Silver layer ETL processes completed with exit code: {exit_code}")
    sys.exit(exit_code)
